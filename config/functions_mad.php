...
function getRaids()
{
    ...
    $raid_move1    = json_decode(file_get_contents('../json/moves.json'), true);
    $raid_move2    = json_decode(file_get_contents('../json/moves.json'), true);
    require('config.php');
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<tr>";
            echo "<td> <img height='42' width='42' src='" . $assetRepo . "pokemon_icon_";
            if ($row["pokemon_id"] < 100 && $row["pokemon_id"] > 9) {
                echo '0' . $row["pokemon_id"];
            } elseif ($row["pokemon_id"] < 10) {
                echo '00' . $row["pokemon_id"];
            } else {
                echo $row["pokemon_id"];
            }
            ;
            echo "_00.png'> " . $raid_mon_name[$row["pokemon_id"]]['name'] . "</td>";
            echo "<td> " . $row["name"] . "</td>";
            echo "<td> " . $raid_move1[$row["move_1"]]['name'] . " & " . $raid_move1[$row["move_2"]]['name'] . "</td>";
            echo "<td> " . $row["cp"] . "</td>";
